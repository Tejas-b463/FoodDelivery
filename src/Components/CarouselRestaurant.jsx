      ...
      <Link key={res.info.id}  to={"/restaurants/"+res.info.id}>
        <div key={res.info.id} className=" top-0 w-[19.9rem]  rounded-lg left-0 p-4 ">
          <div className="overflow-hidden relative">
            ...
            <div className="py-2">
            <h1 className="text-lg font-semibold w-12 verflow-hidden text-ellipsis whitespace-nowrap  leading-tight text-gray-900">{res.info.name}</h1>
            <p className="flex items-center gap-1">{res.info.avgRating} <PiStarThin className="text-green-700"/></p>
            <p className="text-gray-400 w-64">{res.info.cuisines.join(", ")}</p>
            </div>
        </div>
        </Link>
    ))
