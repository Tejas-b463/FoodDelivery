        ...
        <ScrollingCarousel>
          {carouselRestaurant?.map((res) => (
            <Link key={res.info.id} to={"/restaurants/" + res.info.id}>
              <div
                key={res.info.id}
                className=" top-0 w-[19.9rem]  rounded-lg left-0 p-4 "
              >
                ...
                <div className="py-2">
                  <h1 className="overflow-hidden text-lg whitespace-nowrap w-[92%] leading-tight">
                    {res.info.name}
                  </h1>
                  <p className="flex items-center gap-1">
                    {res.info.avgRating}{" "}
                    <PiStarThin className="text-green-700" />
                  </p>
                  <p className="overflow-hidden text-ellipsis whitespace-nowrap w-[100%] leading-tight text-gray-500">
